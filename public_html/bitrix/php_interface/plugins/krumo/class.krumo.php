...
Class krumo
{
    ...
    Function phpini()
    {
        ...
        <div class="krumo-title">
            ...
            <code><b><?php echo get_cfg_var('cfg_file_path'); ?></b></code>.
        </div>
        <?php
            ...
            <ul class="krumo-node krumo-first">
                ...
                <li class="krumo-footnote">
                    ...
                    <?php if (@$d['file']) {
                        ...
                        $fopen = @file($d['file']);
                        $code = '';
                        for ($i = $d['line'] - 3; $i <= $d['line'] + 2; $i++) {
                            $line = $i + 1;
                            $code .= (($line == $d['line'])?'*':' ') . " $line| " . $fopen[$i];
                        }

                        ?>
                        <div class="krumo-element krumo-expand " onclick=" showHide('krumo-source-<?=$d['line']?>')">Source code</div>
                        <div class="krumo-preview" id="krumo-source-<?=$d['line']?>" style="display: none">
                            <code><?php echo highlight_string("<?php Called from:{$d['file']}:{$d['line']}" . PHP_EOL . PHP_EOL . $code . PHP_EOL . '?>', true) ?></code>
                        </div>
                    <?php } ?>
                    &nbsp;
                </li>
